...
bool CFSFolder::LoadComments()
{
  ...
  if (!file.Open(path + kDescriptionFileName))
    return false;
  UInt64 length;
...
bool CFSFolder::SaveComments(UString str)
{
  NIO::COutFile file;
  UString path;
  GetProgramFolderPath(path);
  if (!file.Create(path + kDescriptionFileName, true))
    return false;
  UString unicodeString;
    ...
    const PROPVARIANT *value, IProgress * /* progress */)
{
  ...
  UString filename =GetRelPath(fi);
