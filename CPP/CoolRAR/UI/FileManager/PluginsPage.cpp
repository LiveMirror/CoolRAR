...
bool CPluginsPage::OnInit()
{
  ...
  UINT32 newFlags =  LVS_EX_FULLROWSELECT;
...
STDMETHODIMP CPluginOptionsCallback::GetRegistryCUPath(BSTR *value)
{
  return StringToBstr(UString(L"Software"
    WSTRING_PATH_SEPARATOR L"CoolRAR"
    WSTRING_PATH_SEPARATOR L"CoolRAR"
...
void CPluginsPage::OnButtonOptions()
{
  ...
  {
    MessageBoxW(HWND(*this), L"There are no options", L"CoolRAR", 0);
    return;
  }
  ...
  {
    MessageBoxW(HWND(*this), L"Can't load plugin", L"CoolRAR", 0);
    return;
  }
  ...
  {
    MessageBoxW(HWND(*this), L"Incorrect plugin", L"CoolRAR", 0);
    return;
  }
