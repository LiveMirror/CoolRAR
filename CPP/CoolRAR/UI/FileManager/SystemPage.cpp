...
static CIDLangPair kIDLangPairs[] =
{
  ...
  { IDC_SYSTEM_SETADVANCED,        0x03000337}
};
...
LONG CSystemPage::OnApply()
{
  ...
  for (int i = 0; i < _extDatabase.ExtBigItems.Size(); i++)
  {
    ...
    {
      UString title = UString(L"¿áÑ¹ ")+extInfo.Ext + UString(L" Ñ¹ËõÎÄ¼þ");
	  UString Multivolume =L"·Ö¾í";
	  if (extInfo.Ext == L"001")
	  {
		  title = UString(L"¿áÑ¹ ")+Multivolume+ UString(L" Ñ¹ËõÎÄ¼þ");
	  }
      UString command = GetProgramCommand();
      UString iconPath;
      Int32 iconIndex = -1;
...
void CSystemPage::SetAdvance()
{
	CApp g_App;
	g_App.SetAdvancedValue();
}
