...
#define MyBrowseSaveFilePath(h, title, initialFolder, s, resultPath) \
	MyGetSaveFilePath(h, title, initialFolder, s, resultPath)	

#define MyBrowseForAppointFile(h,title, inititalFolder,s, resultpath) \
  AllOpenFileName(h,title, inititalFolder,s, resultpath)//浏览指定格式的文件  指定.reg格式例子WCHAR s[MAX_PATH]=L"*.reg\0*.reg\0所有文件\0*.*\0" 
#endif
