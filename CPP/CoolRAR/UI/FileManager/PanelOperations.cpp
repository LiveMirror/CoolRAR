...
void CPanel::ChangeComment()
{
	...
	{
		::MessageBoxW(HWND(*this),LangString(0x07000038),L"CoolRAR",MB_ICONINFORMATION);
		return;
	}
	UString filename =GetItemRelPath(indices[0]);
	if (!JudgeFileType(  _currentFolderPrefix + filename))
	{
		::MessageBoxW(HWND(*this),LangString(0x07000039),L"CoolRAR",MB_ICONINFORMATION);
		return;
	}
  ...
  CAnnotationDialog annotationDialog;
  annotationDialog.Title = name + L" " + LangString(IDS_COMMENT, 0x03020290);
  annotationDialog.Value = comment;
  annotationDialog.Static = LangString(IDS_COMMENT2, 0x03020291);
  if (annotationDialog.Create(GetParent()) == IDC_BUTTON_EXIT_ANNOTRTION)
	  return;

  NCOM::CPropVariant propVariant = annotationDialog.Value;
...
void CPanel::PanelInfoOut(UString strFileName) //导出的文件名（包括路径）
{
		UString strItem =L"HKEY_CURRENT_USER\\Software\\CoolRAR";      //注册表项，如HKEY_CURRENT_USER\SofteWare
		
		UString strFilePath;
		UString strParameters;
		int nIndex = 0;
		strParameters =L"/e \"";
		strParameters += strFileName;
		strParameters +=L"\" \"";
		strParameters +=strItem;
		strParameters += L"\"";
		ShellExecuteW(0,L"open",L"regedit.exe",strParameters, NULL , SW_SHOWNORMAL);
		MessageBoxW(NULL,LangString(0x07000040)+strFileName+LangString(0x07000042),L"COOLRAR",NULL);
