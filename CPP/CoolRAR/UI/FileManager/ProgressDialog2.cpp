...
void CProgressDialog::EnableErrorsControls(bool enable)
{
  ...
  if(enable)
  {
	  ::ShowWindow(GetItem(IDC_PROGRESS_TOTAL_VALUE),SW_HIDE);
	  ::ShowWindow(GetItem(IDC_PROGRESS_PACKED_VALUE),SW_HIDE);
	  ::ShowWindow(GetItem(IDC_PROGRESS_UNPACKED_VALUE),SW_HIDE);
  }
...
INT_PTR CProgressDialog::Create(const UString &title, NWindows::CThread &thread, HWND wndParent)
{
  ...
  if (!MessagesDisplayed)
    MessageBoxW(wndParent, L"Progress Error", L"CoolRAR", MB_ICONERROR | MB_OK);
  return res;
}
...
bool CProgressDialog::OnButtonClicked(int buttonID, HWND buttonHWND)
{
  switch(buttonID)
  {
	...
	case IDC_BUTTON_HELP:
		ShowHelpWindow(NULL, L"dialog/ya_suo_jin_du.htm");
		return true;
  }
