...
static const wchar_t *kDefaultSfxModule = L"CoolRAR.sfx";
    ...
    CUpdateOptions &options, CUpdateCallbackGUI *callback, HWND hwndParent)
{
  ...
  NOptinsAddDlg::CInfo &di = optionDlg.Info;
  optionDlg.ArcFormats = &codecs->Formats;
  ...
  for (int i = 0; i < codecs->Formats.Size(); i++)
  {
	...
	{
		if (ai.Name.CompareNoCase(L"7z") == 0 ||ai.Name.CompareNoCase(L"zip") == 0 ||ai.Name.CompareNoCase(L"tar") == 0  )
		{
			optionDlg.ArcIndices.Add(i);
		}
     
	}
  }

  if (optionDlg.ArcIndices.Size() == 0)
  {
  ...
  optionDlg.OriginalFileName = options.ArchivePath.Prefix + fileInfo.Name;
   
  di.CurrentDirPrefix = currentDirPrefix;
  di.SFXMode = options.SfxMode;
  di.OpenShareForWrite = options.OpenShareForWrite;
  di.deleteSourceFile =options.Sourcefiledelete;
  di.CommentsValue=options.ValusComments;
  ...
  INT_PTR res = optionDlg.OptionsAddDialog(hwndParent, g_hInstance);

  if(res != 1) return E_ABORT; 
  ...
  options.Sourcefiledelete =di.deleteSourceFile;
  options.ValusComments=di.CommentsValue;
