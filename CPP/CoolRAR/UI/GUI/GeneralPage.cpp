...
bool CGeneralPage::OnInit()
{
	...
	m_ArchivePath.Attach(GetItem(IDC_GEN_PAGE_COMBO_FILE));//文件名
	...
	for (i = 0; i < Delete->ArcIndices.Size(); i++)
	{
		...
		if(ai.Name == L"7z" || ai.Name == L"tar" || ai.Name == L"zip"  )
		{
			...
			if (ai.Name.CompareNoCase(Delete->m_RegistryInfo.ArcType) == 0)
			{
				SetFormat(i);
...
void CGeneralPage::SetVolumeSize()
{
	...
	if (!volumeString.IsEmpty())
	{
		if (!ParseVolumeSizes(volumeString, Delete->Info->VolumeSizes))
		{
			...
			MessageBoxW(*this,LangString(0x07000050),LangString(0x07000051),NULL);
			m_Volume.SetText(L"");
			SetVolumeSize();
			return;
		}
