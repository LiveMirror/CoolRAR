...
static LPCWSTR kPressItselfFrom   = L" -sfx";
static LPCWSTR kPressPassWord     = L" -p";

static UString defaultPassWord;  //默认密码
...
{
  MessageBoxW(g_HWND, message, L"CoolRAR", MB_ICONERROR | MB_OK);
}
...
void SaveDefaultPassword(UString &savepassword)
{
	savepassword.Empty();
...
static HRESULT ExtractGroupCommandWaitFinish(const UStringVector &arcPaths, UString &params)
{
	AddLagePagesSwitch(params);
	params += kArcIncludeSwitches;
	CFileMapping fileMapping;
	NSynchronization::CManualResetEvent event;
	RINOK(CreateMap(arcPaths, fileMapping, event, params));
	return MyCreateProcess(Get7zGuiPath(), params, 0, true, &event);
}
void ExtractDricet(const UStringVector &arcPaths, const UString &outFolder, bool showDialog)
{
	ExtractArchives(arcPaths, outFolder,showDialog);
}
...
HRESULT ExtractArchives(const UStringVector &arcPaths, const UString &outFolder, bool showDialog,bool waitfinish)
{
	...
	if (showDialog)
		params += kShowDialogSwitch;
	return ExtractGroupCommandWaitFinish(arcPaths, params);
	MY_TRY_FINISH
}
