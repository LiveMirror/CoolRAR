...
#ifndef _UNICODE
UStringVector CFindFile::FindDirFile(UString FilePath)
{

	CFileInfoW fileinfo;
	UString strPath= FilePath+L"*.*";
	CFindFile findfile;
	bool  bWork = findfile.FindFirst(strPath,fileinfo);
	UStringVector VectorName;
	while(bWork)
	{	
		if(fileinfo.IsDir()&& fileinfo.Name != L"." && fileinfo.Name != L".." && !fileinfo.IsHidden())
		{
			VectorName.Add(fileinfo.Name);
		}
...
bool GetDriveInfo(UString &driveName) 
{
	...
	WCharToMByte(driveName.GetBuffer(driveName.Length()),szBuffer,sizeof

		(szBuffer)/sizeof(szBuffer[4096]));
	...
	if (NFile::NSystem::MyGetDriveType(driveName) == DRIVE_FIXED)
	{
		...
		if(NFile::NSystem::MyGetDriveType(driveName) == DRIVE_CDROM )
		{
