...
bool AllOpenFileName(HWND hwnd, LPCWSTR title, LPCWSTR fullFileName,
					 LPCWSTR s ,UString &resPath
#ifdef UNDER_CE
					 , bool openFolder
#endif
					 )
{
	...
	if (!g_IsNT)
	{
		...
		info.hInstance = 0;
		const int kFilterBufferSize = MAX_PATH;
		CHAR c[kFilterBufferSize];

		WideCharToMultiByte(CP_ACP,1,s,1,c,kFilterBufferSize,0,0);
		info.lpstrFilter = c;
...
#endif
	{
		...
		info.lpstrFilter = s;//L"*.theme.7z\0*.theme.7z\0所有文件\0*.*\0"//
...
bool MyGetSaveFilePath(HWND hwnd, LPCWSTR title, LPCWSTR fullFileName,
					   LPCWSTR s, UString &resPath
#ifdef UNDER_CE
					   , bool openFolder
#endif
					   )
{
	...
	if (!g_IsNT)
	{
		...
		bool res = BOOLToBool(::GetSaveFileNameA(&info));
		resPath = GetUnicodeString(buffer);
		return res;
	}
	else
#endif
	{
		...
		bool res = BOOLToBool(::GetSaveFileNameW(&info));
		resPath = buffer;
		return res;
	}
}
